    ...
    public QuestionItemDTO save(QuestionItemDTO questionItemDTO) {
        ...
        for(Long frozenTubeId :questionItemDTO.getFrozenTubeIds()){
            ...
            if(questionItemDetails == null){
                if(!frozenTube.getFrozenTubeState().equals(Constants.FROZEN_BOX_STOCKED)
                    && !frozenTube.getFrozenTubeState().equals(Constants.FROZEN_BOX_TRANSHIP_COMPLETE)
                    && !frozenTube.getFrozenTubeState().equals(Constants.FROZEN_BOX_PUT_SHELVES)
                    && !frozenTube.getFrozenTubeState().equals(Constants.FROZEN_BOX_STOCKING)){
                    throw new BankServiceException("样本不在入库阶段，不能创建问题！");
                }
